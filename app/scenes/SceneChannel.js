...
SceneSceneChannel.STATE_LOADING_CINFO = 0;
SceneSceneChannel.STATE_LOADING_TOKEN = 1;
SceneSceneChannel.STATE_LOADING_VIEWURL = 2;
SceneSceneChannel.STATE_LOADING_PLAYLIST = 3;
SceneSceneChannel.STATE_PLAYING = 4;
SceneSceneChannel.state = SceneSceneChannel.STATE_LOADING_CINFO;

SceneSceneChannel.QualityAuto = "원본화질";
...
SceneSceneChannel.channelResponse;
SceneSceneChannel.tokenResponse;
SceneSceneChannel.viewUrlResponse;
...
SceneSceneChannel.prototype.handleFocus = function () {
    ...
    SceneSceneChannel.channelResponse = 0;
    SceneSceneChannel.tokenResponse = 0;
    SceneSceneChannel.viewUrlResponse = 0;
...
SceneSceneChannel.qualityChanged = function()
{
	...
	SceneSceneChannel.playingUrl = SceneSceneChannel.viewUrlResponse.view_url + '?aid=' + SceneSceneChannel.tokenResponse.CHANNEL.AID;
...
SceneSceneChannel.updateStreamInfo = function()
{
		...
		{ 
			if (xmlHttp.status === 200)
			{
				try
				{
					...
					$("#stream_info_title").text(response.CHANNEL.TITLE);
					//$("#stream_info_viewer").text(addCommas(response.stream.viewers) + ' ' + STR_VIEWER);
					$("#stream_info_icon").attr("src", 'http://stimg.afreecatv.com/LOGO/' + response.CHANNEL.BJID.substring(0, 2)+ '/' + response.CHANNEL.BJID + '/' + response.CHANNEL.BJID + '.jpg');
				}
				catch (err)
				{
	...
	data = 'bid=' + SceneSceneBrowser.selectedChannel + '&bno=&pwd=&type=&player_type=html5&stream_type=common&quality=&mode=landing';
    xmlHttp.open("POST", 'http://live.afreecatv.com/afreeca/player_live_api.php', true);
	xmlHttp.timeout = 10000;
	xmlHttp.withCredentails = true;
...
SceneSceneChannel.loadDataSuccess = function(responseText)
{
	...
	if (SceneSceneChannel.state == SceneSceneChannel.STATE_LOADING_CINFO)
	{
		SceneSceneChannel.channelResponse = $.parseJSON(responseText);
		SceneSceneChannel.state = SceneSceneChannel.STATE_LOADING_TOKEN;
		SceneSceneChannel.loadData();
	}
	else if (SceneSceneChannel.state == SceneSceneChannel.STATE_LOADING_TOKEN)
	{
		SceneSceneChannel.tokenResponse = $.parseJSON(responseText);
		SceneSceneChannel.state = SceneSceneChannel.STATE_LOADING_VIEWURL;
		SceneSceneChannel.loadData();
	}
	else if (SceneSceneChannel.state == SceneSceneChannel.STATE_LOADING_VIEWURL)
	{
		SceneSceneChannel.viewUrlResponse = $.parseJSON(responseText);
...
SceneSceneChannel.loadDataRequest = function()
{
	try
	{
		...
		if (SceneSceneChannel.state == SceneSceneChannel.STATE_LOADING_CINFO)
		{
			theUrl = 'http://live.afreecatv.com/afreeca/player_live_api.php';
			data = 'bid=' + SceneSceneBrowser.selectedChannel + '&bno=&pwd=&type=&player_type=html5&stream_type=common&quality=&mode=landing';
			// data = 'bid=0987po&bno=&pwd=&type=&player_type=html5&stream_type=common&quality=&mode=landing';
			//alert(SceneSceneBrowser.selectedChannel);
		}
		else if (SceneSceneChannel.state == SceneSceneChannel.STATE_LOADING_TOKEN)
		{
			theUrl = 'http://live.afreecatv.com/afreeca/player_live_api.php';
			data = 'bid=' + SceneSceneChannel.channelResponse.CHANNEL.BJID + '&bno=' + SceneSceneChannel.channelResponse.CHANNEL.BNO + '&quality=original&pwd=&type=pwd';
		}
		else if (SceneSceneChannel.state == SceneSceneChannel.STATE_LOADING_VIEWURL)
		{
			theUrl = SceneSceneChannel.channelResponse.CHANNEL.RMD + '/broad_stream_assign.html?return_type=gs_cdn&use_cors=true&cors_origin_url=play.afreecatv.com&broad_key=' + SceneSceneChannel.channelResponse.CHANNEL.BNO + '-flash-original-hls&time=';
		}
		else if (SceneSceneChannel.state == SceneSceneChannel.STATE_LOADING_PLAYLIST)
		{
			theUrl = SceneSceneChannel.viewUrlResponse.view_url + '?aid=' + SceneSceneChannel.tokenResponse.CHANNEL.AID;
		}
		...
		if (SceneSceneChannel.state == SceneSceneChannel.STATE_LOADING_CINFO || SceneSceneChannel.state == SceneSceneChannel.STATE_LOADING_TOKEN)
		{
