...
SceneSceneBrowser.createCell = function(row_id, coloumn_id, data_name, thumbnail, title, info, info2, info_fill)
{
	...
	return $('<td id="cell_' + row_id + '_' + coloumn_id + '" class="stream_cell" data-channelname="' + data_name + '"></td>').html(
			'<img id="thumbnail_' + row_id + '_' + coloumn_id + '" class="stream_thumbnail" src="' + "http:" + thumbnail + '"/> \
...
SceneSceneBrowser.loadDataError = function()
{
	...
	{
		SceneSceneBrowser.loadingData = false;
		SceneSceneBrowser.showDialog("Error: to load stream data.");
	}
};
...
SceneSceneBrowser.loadDataSuccess = function(responseText)
{
	...
	// alert(response);
	eval(response);
	// alert(aBroadList.broad);
	
	var response_items = aBroadList.cnt;
	/*
	...
	*/
	...
	{        
    	...
    	for (t = 0; t < SceneSceneBrowser.ColoumnsCount && cursor < response_items; t++, cursor++)
    	{
            ...
            */
    		var stream = aBroadList.broad[cursor];
    		
    		// alert(stream.broad_thumb);
    		
    		cell = SceneSceneBrowser.createCell(row_id, t, stream.user_id, stream.broad_thumb, stream.broad_title, stream.station_name, addCommas(stream.total_view_cnt) + ' 명 시청', false);
...
SceneSceneBrowser.loadDataRequest = function()
{
	try
	{
		...
		else
		{
			...
			theUrl = 'http://static.file.afreecatv.com/pc/ko_KR/main_broad_list.js';
		}
